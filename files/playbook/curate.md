...
6. **For a link with real content, write the long form to a file.** Save the fetched
   `text` (plus the source line) to `/notes/<record_id>.md` and set the row's
   `content_path` to it. This is what makes the link searchable later (RAG) — the
   `body` only needs the gist. For a short plain thought, skip the file; the `body` is
   enough.

7. **Write it all back** in one update: `title`, `tags`, `kind`, `collection_id` (or
   leave null for Unsorted), `content_path` (if any), `preview_image_url` (the fetched
   `image_url`, if any), a tightened `body` (a 1–2 sentence gist for links; leave a
   plain thought's body as-is), and `status: filed` (or `failed` per step 3).
